...
export default function Checkout() {
  ...
  // 🛠 Ensure client-side only state access (prevents SSR mismatch)
  useEffect(() => {
    setCartLoaded(true);
  }, []);
        ...
        <div className="col-lg-5">
          <div className="card shadow-sm p-4 order-summary sticky-top">
            ...
            {state.cart.length > 0 ? (
              state.cart.map((item) => (
                <div key={item.id} className="d-flex align-items-center mb-3">
                  ...
                  <div className="ms-auto fw-bold">₹{item.price}</div>
                </div>
              ))
            ) : (
            ...
            <p className="text-muted small text-center mt-3">
              🔒 100% Secure Payments | 🚚 Free Shipping | 💄 Dermatologist Approved
            </p>
          </div>
        </div>
      </div>
